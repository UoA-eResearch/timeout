...
def compute_counts():
    ...
    # Post-level stage flags: every figure number is a filter on this frame
    keep_cols = [col for col in ["id", "extractor", "channel", "uploader", "title",
                                 "webpage_url", "upload_date", "duration", "like_count",
                                 "view_count", "comment_count", "supplements"]
                 if col in df.columns]
    stages = df[keep_cols].copy()
    stages["menopause_targeted"] = df["menopause"] == True
    dropped = meno.index.difference(uniq.index)
    stages["cross_platform_duplicate"] = pd.Series(pd.NA, index=df.index, dtype="boolean")
    stages.loc[meno.index, "cross_platform_duplicate"] = False
    stages.loc[dropped, "cross_platform_duplicate"] = True
    ct_all = df["content_type"].astype(str).str.strip().str.lower()
    div1_map = dict(DIV1_LABELS)
    stages["division1_content_type"] = ct_all.map(div1_map).fillna("Unclassified")
    stages["mentions_supplements"] = df["supplements"].apply(
        lambda v: len(split_supplements(v)) > 0)
    in_supp = stages["menopause_targeted"] & (stages["cross_platform_duplicate"] == False) \
        & stages["mentions_supplements"]
    stages["in_final_supplement_set"] = in_supp
    stages["division2_primary_category"] = df["supplements"].where(in_supp).apply(
        lambda v: primary_category(v) if isinstance(v, str) else None)
    stages["division3_primary_target"] = df["supplements"].where(in_supp).apply(
        lambda v: primary_target(v) if isinstance(v, str) else None)
...
def right_label(s, n, halo=False):
    ...
    s.append(f'<text x="{n["x"] + NODE_WIDTH + 8}" y="{cy:.1f}" class="{cls}">'
             f'<tspan class="ncount-inline">{fmt(n["val"])}</tspan>  {label}</text>')
...
def write_stages_workbook(c):
    """Post-level export: every figure number is a filter on the posts sheet.

    Each row is one LLM-processed post with a flag or code per pipeline
    stage; the recipes sheet states the exact filter behind every Sankey
    node so any figure can be traced to its specific posts.
    """
    stages = c["stages"]
    recipes = [
        ("Downloaded & LLM-processed", "all rows", len(stages)),
        ("Menopause-targeted", "menopause_targeted == True",
         int(stages["menopause_targeted"].sum())),
        ("Cross-platform duplicates", "cross_platform_duplicate == True",
         int((stages["cross_platform_duplicate"] == True).sum())),
        ("Unique posts", "menopause_targeted & ~cross_platform_duplicate",
         int((stages["menopause_targeted"] & (stages["cross_platform_duplicate"] == False)).sum())),
    ]
    uniq_mask = stages["menopause_targeted"] & (stages["cross_platform_duplicate"] == False)
    for label, val in c["div1"]:
        recipes.append((f"Division 1: {label}",
                        f"unique posts & division1_content_type == '{label}'",
                        int((uniq_mask & (stages["division1_content_type"] == label)).sum())))
    recipes.append(("Mentions supplements", "in_final_supplement_set == True",
                    int(stages["in_final_supplement_set"].sum())))
    recipes.append(("No supplements mentioned", "unique posts & ~mentions_supplements",
                    int((uniq_mask & ~stages["mentions_supplements"]).sum())))
    for name, val in c["categories"]:
        recipes.append((f"Division 2: {CATEGORY_SHORT.get(name, name)}",
                        f"division2_primary_category == '{name}'",
                        int((stages["division2_primary_category"] == name).sum())))
    for name, val in c["targets"]:
        recipes.append((f"Division 3: {TARGET_SHORT.get(name, name)}",
                        f"division3_primary_target == '{name}'",
                        int((stages["division3_primary_target"] == name).sum())))
    recipes_df = pd.DataFrame(recipes, columns=["Sankey node", "Filter on posts sheet", "Posts"])

    out = Path("data/pipeline_stages.xlsx")
    with pd.ExcelWriter(out) as writer:
        stages.to_excel(writer, sheet_name="posts", index=False)
        recipes_df.to_excel(writer, sheet_name="recipes", index=False)
    print(f"Saved {out} ({len(stages)} posts, {len(recipes_df)} node recipes)")
